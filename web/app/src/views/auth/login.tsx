...
import { IconGitHub1 } from '@niuniu-wiki/icons';
  ...
  return (
    <>
      <Box
        sx={theme => ({
          position: 'relative',
          minHeight: mobile ? `calc(100vh - 84px)` : `calc(100vh - 40px)`,
          display: 'flex',
          alignItems: 'center',
          ...
          backgroundImage: `radial-gradient(circle at 18% 10%, ${alpha(theme.palette.primary.main, 0.18)}, transparent 31rem), radial-gradient(circle at 86% 88%, ${alpha('#af52de', 0.14)}, transparent 30rem)`,
          '&::before': {
            content: '""',
            position: 'absolute',
            width: 360,
            ...
            bottom: '-22%',
            borderRadius: '50%',
            background: `linear-gradient(145deg, ${alpha(theme.palette.primary.main, 0.12)}, ${alpha('#af52de', 0.08)})`,
            filter: 'blur(2px)',
          },
        })}
      >
        <Box
          ...
          sx={theme => ({
            ...
            bgcolor: alpha(
              ...
              themeMode === 'dark' ? 0.86 : 0.78,
            ),
            border: `1px solid ${alpha(theme.palette.text.primary, 0.09)}`,
            borderRadius: '30px',
            boxShadow: `0 30px 100px ${alpha(theme.palette.text.primary, 0.14)}, inset 0 1px 0 ${alpha('#fff', 0.38)}`,
            ...(mobile && {
              mx: 1,
            }),
          })}
        >
          <Stack alignItems='center'>
            <Stack alignItems='center' gap={1} sx={{ mb: 4.5 }}>
              ...
              >
                完成认证，继续访问「{kbDetail?.settings?.title}」
              </Box>
            </Stack>
            {authType === ConstsAuthType.AuthTypeSimple && (
              <>
                ...
                <Button
                  ...
                  variant='contained'
                  onClick={handleLogin}
                  sx={{ mt: 3, height: '50px', fontSize: 15 }}
