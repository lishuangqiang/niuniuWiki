  ...
  return (
    <Box
      ...
      sx={theme => ({
        ...
        p: mobile ? '28px 22px 42px' : { md: 5, lg: '64px 72px 76px' },
        mb: mobile ? 2 : 4,
        minHeight: mobile ? 'auto' : 'calc(100vh - 160px)',
        bgcolor: alpha(
        ...
        boxShadow: mobile
          ? `0 10px 40px ${alpha(theme.palette.text.primary, 0.045)}`
          : `0 24px 80px ${alpha(theme.palette.text.primary, 0.075)}`,
      })}
    >
      <DocMetaInfo
        info={info}
      ...
      {info?.meta?.summary && (
        <Box
          sx={theme => ({
            ...
            borderRadius: '20px',
            background: `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.075)}, ${alpha('#af52de', 0.045)})`,
            p: { xs: 2.25, md: 2.75 },
            fontSize: 14.5,
            lineHeight: 1.85,
          })}
        >
          ...
          <Box sx={{ color: 'text.secondary' }}>{info?.meta?.summary}</Box>
        </Box>
      )}
