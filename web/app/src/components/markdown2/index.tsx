...
interface MarkDown2Props {
  ...
  onReferenceClick?: (reference: ChunkResultItem, index: number) => void;
}
...
const MarkDown2: React.FC<MarkDown2Props> = ({
  ...
  const customizeRenderer = useCallback(
    (md: MarkdownIt) => {
      ...
      // 将模型回答中的 [文档 N] 转换为可交互引用。
      md.renderer.rules.text = (tokens, idx) => {
        const escaped = md.utils.escapeHtml(tokens[idx].content);
        return escaped.replace(/\[文档\s*(\d+)\]/g, (matched, number) => {
          const referenceIndex = Number(number) - 1;
          const reference = references[referenceIndex];
          if (!reference) return matched;
          const active = reference.node_id === activeReferenceId;
          return `<button type="button" class="doc-citation${active ? ' is-active' : ''}" data-reference-index="${referenceIndex}" aria-label="查看引用文档 ${md.utils.escapeHtml(reference.name || String(number))}">[文档 ${number}]</button>`;
        });
      };
      ...
      md.renderer.rules.link_open = (tokens, idx) => {
        ...
        // 历史回答中可能保存了旧域名或缺少端口的内部文档链接。
        // 根据 node_id 将它归一化为当前站点相对地址，并交给引用工作区处理。
        const nodeMatch = href.match(/\/node\/([^/?#]+)/);
        const nodeId = nodeMatch ? decodeURIComponent(nodeMatch[1]) : undefined;
        const referenceIndex = nodeId
          ? references.findIndex(reference => reference.node_id === nodeId)
          : -1;
        if (referenceIndex >= 0) {
          const reference = references[referenceIndex];
          const active = reference.node_id === activeReferenceId;
          const normalizedHref = `${basePath}/node/${encodeURIComponent(reference.node_id)}`;
          return `<a href="${md.utils.escapeHtml(normalizedHref)}" class="doc-reference-link${active ? ' is-active' : ''}" data-reference-index="${referenceIndex}" target="_blank" rel="noopener noreferrer">`;
        }

        token.attrSet('target', '_blank');
        token.attrSet('rel', 'noopener noreferrer');

        return `<a href="${md.utils.escapeHtml(href)}" target="_blank" rel="noopener noreferrer" style="color: ${theme.palette.primary.main}; text-decoration: underline;">`;
      };
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      const citation = target.closest('.doc-citation') as HTMLButtonElement;
      if (citation) {
        e.preventDefault();
        const referenceIndex = Number(citation.dataset.referenceIndex);
        const reference = references[referenceIndex];
        if (reference) onReferenceClick?.(reference, referenceIndex);
        return;
      }

      const referenceLink = target.closest(
        'a.doc-reference-link',
      ) as HTMLAnchorElement;
      if (referenceLink) {
        e.preventDefault();
        const referenceIndex = Number(referenceLink.dataset.referenceIndex);
        const reference = references[referenceIndex];
        if (reference) onReferenceClick?.(reference, referenceIndex);
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!container) return;
    container
      .querySelectorAll<HTMLButtonElement>('.doc-citation')
      .forEach(citation => {
        const index = Number(citation.dataset.referenceIndex);
        citation.classList.toggle(
          'is-active',
          references[index]?.node_id === activeReferenceId,
        );
      });
