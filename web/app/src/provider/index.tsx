...
import { NiuniuWikiContractProApiShareV1AuthInfoResp } from '@/request/pro/types';
import {
  filterEmptyFolders,
  convertToTree,
  findNavIdByNodeId,
...
interface StoreContextType {
  authInfo?: NiuniuWikiContractProApiShareV1AuthInfoResp;
  widget?: WidgetInfo;
  kbDetail?: KBDetail;
  catalogShow?: boolean;
  tree?: ITreeItem[];
  ...
  qaDraftFiles?: File[];
  setQaDraftFiles?: Dispatch<SetStateAction<File[]>>;
  qaConversationTarget?: { id: string; nonce: string };
  setQaConversationTarget?: Dispatch<
    SetStateAction<{ id: string; nonce: string } | undefined>
  >;
  /** 栏目列表，多栏目时展示导航栏 */
  navList?: NavItem[];
  /** 当前选中的栏目 id */
...
export default function StoreProvider({
  ...
  const NAV_ID_STORAGE_KEY = 'niuniu-wiki-selected-nav-id';
  ...
  const [qaDraftFiles, setQaDraftFiles] = useState<File[]>([]);
  const [qaConversationTarget, setQaConversationTarget] = useState<{
    id: string;
    nonce: string;
  }>();
