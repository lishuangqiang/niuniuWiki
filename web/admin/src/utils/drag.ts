...
const normalizeRagStatus = (
  status?: ConstsNodeRagInfoStatus | string,
): ConstsNodeRagInfoStatus | undefined => {
  const normalized = status?.trim().toUpperCase();
  switch (normalized) {
    case 'PENDING':
    case 'WAITING':
      return ConstsNodeRagInfoStatus.NodeRagStatusPending;
    case 'RUNNING':
    case 'PROCESSING':
      return ConstsNodeRagInfoStatus.NodeRagStatusRunning;
    case 'FAILED':
    case 'FAILURE':
    case 'ERROR':
      return ConstsNodeRagInfoStatus.NodeRagStatusFailed;
    case 'SUCCESS':
    case 'SUCCEEDED':
    case 'COMPLETED':
      return ConstsNodeRagInfoStatus.NodeRagStatusSucceeded;
    case 'REINDEX':
    case 'REINDEXING':
      return ConstsNodeRagInfoStatus.NodeRagStatusReindexing;
    default:
      return undefined;
  }
};
...
export function convertToTree(data: DomainNodeListItemResp[]) {
  ...
  data.forEach(item => {
    const node: ITreeItem = {
      ...
      type: item.type!,
      rag_status: normalizeRagStatus(item.rag_info?.status),
